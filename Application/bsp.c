...
void SysTick_Handler(void) {
    ...
    QF_TICK_X(0U, (void *) 0); /* process time events (tick rate 0) */
...
void GPIOPortF_IRQHandler(void) {
    ...
    if ((GPIOF_AHB->RIS & BTN_SW1) != 0U){ /* iterrupt coming from the botton switch */
...
void BSP_init(void) {
    ...
    /* LEDs: PF1/PF2/PF3 outputs, digital enable */
    ...
    GPIOF_AHB->PUR |= (BTN_SW1); /* pull up resistor unable */

    /* GPIO interrupt setup for the SW1 */
    GPIOF_AHB->IS  &= ~(BTN_SW1);    /* edge-triggered */
    GPIOF_AHB->IBE &= ~(BTN_SW1);    /* single-edge */
    GPIOF_AHB->IEV &= ~(BTN_SW1);    /* falling edge */
...
void QF_onStartup(void){
    ...
    /* set the interrupt priorities of 'kernel aware' */
...
void QF_onCleanup(void){
    /* optional: turn off peripherals, etc. */
}

void QXK_onIdle(void){
    /* Optional heartbeat: a quick PF1 toggle to show idle activity */
    GPIOF_AHB->DATA_Bits[LED_RED] = LED_RED;
    GPIOF_AHB->DATA_Bits[LED_RED] = 0U;
